  ...
  return (
    <div ref={containerRef} className="bg-black text-white min-h-screen selection:bg-white selection:text-black overflow-hidden">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        >
          <motion.div
            ...
            className="mb-8"
          >
            <span className="text-[10px] tracking-[0.8em] uppercase opacity-40">{t('perfEdge.velocity')}</span>
          </motion.div>
          ...
          <div className="mt-12 flex justify-center gap-20">
            <div className="text-left">
              ...
              <div className="text-[8px] uppercase tracking-[0.3em] opacity-30">{t('perfEdge.targetLatency')}</div>
            </div>
            <div className="text-left">
              <div className="text-4xl font-mono font-light tracking-tighter">100/100</div>
              <div className="text-[8px] uppercase tracking-[0.3em] opacity-30">{t('perfEdge.coreWebVitals')}</div>
            </div>
          </div>
        </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-32 items-center">
            <div>
              <h2 className="text-6xl md:text-8xl font-bold tracking-tighter uppercase mb-12 leading-none">
                {t('perfEdge.globalFabricTitle1')} <br />
                <span className="opacity-30">{t('perfEdge.globalFabricTitle2')}</span>
              </h2>
              <p className="text-2xl text-white/50 leading-relaxed font-light max-w-xl mb-12">
                {t('perfEdge.globalFabricDesc')}
              </p>

              <div className="space-y-12">
                {[
                  { label: t('perfEdge.edgeNodes'), value: "250+", icon: <Globe className="w-4 h-4" /> },
                  { label: t('perfEdge.globalLatency'), value: "< 20ms", icon: <Activity className="w-4 h-4" /> },
                  { label: t('perfEdge.uptime'), value: "99.99%", icon: <Gauge className="w-4 h-4" /> }
                ].map((stat, i) => (
      ...
      <section className="py-60 bg-white text-black">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col items-center text-center mb-32">
            <span className="text-[10px] font-mono mb-8 block opacity-40">{t('perfEdge.systemOptimized')}</span>
            <h3 className="text-7xl md:text-[10vw] font-black tracking-tighter uppercase leading-[0.8]">
              {t('perfEdge.overclockedTitle')}
            </h3>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-1">
            {[
              { title: t('perfEdge.overclockItems.0.title'), desc: t('perfEdge.overclockItems.0.desc'), icon: <Zap /> },
              { title: t('perfEdge.overclockItems.1.title'), desc: t('perfEdge.overclockItems.1.desc'), icon: <Timer /> },
              { title: t('perfEdge.overclockItems.2.title'), desc: t('perfEdge.overclockItems.2.desc'), icon: <BarChart3 /> }
            ].map((item, i) => (
              <motion.div
                key={i}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-end">
            <div>
              <h2 className="text-5xl font-bold tracking-tighter uppercase mb-8">
                {t('perfEdge.engineeringTitle1')} <br />{t('perfEdge.engineeringTitle2')}
              </h2>
              <p className="text-muted-foreground leading-relaxed max-w-md">
                {t('perfEdge.engineeringDesc')}
              </p>
            </div>
            <div className="space-y-8">
              {[
                { label: t('perfEdge.benchmarks.0.label'), value: 100 },
                { label: t('perfEdge.benchmarks.1.label'), value: "0.4s" },
                { label: t('perfEdge.benchmarks.2.label'), value: "< 12kb" }
              ].map((item, i) => (
      ...
      <section className="py-60 text-center border-t border-white/5">
        ...
        >
          <span className="text-[10px] tracking-[0.8em] uppercase opacity-40 mb-8 block">{t('perfEdge.ctaLabel')}</span>
          <h2 className="text-5xl md:text-8xl font-light tracking-tighter uppercase italic">
            {t('perfEdge.ctaTitle1')} <br />
            <span className="font-bold not-italic">{t('perfEdge.ctaTitle2')}</span>
          </h2>
        </motion.div>
      </section>
    </div>
