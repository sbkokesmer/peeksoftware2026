    ...
    <div className="bg-[#080808] text-white min-h-screen font-sans selection:bg-white selection:text-black">
      ...
      <section className="pt-32 pb-20 px-6 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          >
            {t('mobDetail.title1')} <br />
            <span className="text-transparent" style={{ WebkitTextStroke: '1px white' }}>{t('mobDetail.title2')}</span>
          </motion.h1>
          <p className="text-2xl text-white/60 font-light max-w-2xl leading-relaxed">
            {t('mobDetail.description')}
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
            <div className="space-y-16">
              {[Smartphone, Layers, Layout].map((Icon, i) => (
                <div key={i} className="flex gap-8 group">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold uppercase tracking-tighter mb-4">{t(`mobDetail.cards.${i}.title`)}</h3>
                    <p className="text-white/40 leading-relaxed text-sm">{t(`mobDetail.cards.${i}.desc`)}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="relative flex justify-center">
              ...
              <div className="absolute -right-12 top-1/4 bg-white text-black p-6 rounded-xl shadow-2xl">
                <div className="text-xs font-bold uppercase tracking-widest mb-1">{t('mobDetail.frameRate')}</div>
      ...
      <section className="py-32 px-6 bg-white text-black">
        <div className="max-w-7xl mx-auto">
          <div className="mb-20">
            <h2 className="text-4xl font-bold tracking-tighter uppercase mb-4">{t('mobDetail.standardsTitle')}</h2>
            <p className="text-black/40">{t('mobDetail.standardsDesc')}</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {[0, 1, 2].map((i) => (
              <div key={i} className="border-l border-black/10 pl-8">
                <span className="text-[10px] font-bold uppercase tracking-widest mb-4 block">{t(`mobDetail.standards.${i}.label`)}</span>
                <p className="text-sm text-black/60 leading-relaxed">{t(`mobDetail.standards.${i}.text`)}</p>
              </div>
            ))}
          </div>
        </div>
